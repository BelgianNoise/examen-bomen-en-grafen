...
public class GraphOplossing {
	...
	public int[][] Dijkstra(int vanKnoop) {
		...
		while (!ok) {
			...
			for (int i = 0; i < this.gewichtenMatrix.length; i++) {
				if (res[0][i] != Integer.MAX_VALUE) {// doorzoek alle knopen
														// waar nog geen kortste
														// pad voor werd
														// gevinden en die
														// bereikbaar zijn uit
					// knopen waar reeds korste pad voor
					// werd gevonden en zoek hierin de
					// kleinste afstand
	...
	private int[][] initMatrixDijkstra(int vanKnoop) {
		int[][] res = new int[this.gewichtenMatrix.length + 1][this.gewichtenMatrix.length]; // eerste
																								// rij
																								// is
																								// rij
																								// met
		...
		for (int i = 0; i < this.gewichtenMatrix.length; i++)
			res[0][i] = Integer.MAX_VALUE; // eerste rij initialiseren op grote
											// waarde omdat kortste afstand nog
											// niet werd gevonden
		...
		for (int i = 0; i < this.gewichtenMatrix.length; i++) {
			res[i][vanKnoop - 1] = 0; // kolom overeenkomstig vanKnoop op nul
										// zetten
		}
		return res;
	}
